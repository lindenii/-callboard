...
function receiveScheduleOfCurrency(uri) {
    ...
    selectCurrency = $('#typeOfCurrency option:selected').val();
...
function getTodayCurrency(uri,selectCurrency, currentDate) {
    let todayCurr=[];
    let dataNow= $.getJSON(uri + 'ExRates/Rates/Dynamics/' + parseInt(selectCurrency),
        { 'startDate': currentDate  , 'endDate': currentDate })
        .done(function (data) {
            $.each(data, function (key, item) {
                todayCurr.push(item.Cur_OfficialRate);
            });
            let resultChange= $('#summa').val()*todayCurr;
            $('#resultMoney').val(resultChange);
        })
}
...
$('#btnResultMoney').click(function(){
    getTodayCurrency(uri,selectCurrency,currentDate);
});
